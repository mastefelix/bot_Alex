...
class Weather(QWidget):
    ...
    def initUI(self):
        ...
        self.request.setPlaceholderText("Город")
        self.info = QTextEdit()
        self.info.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.info.setReadOnly(True)
        self.button_start = QPushButton("Узнать погоду")
    ...
    def connects(self):
        ...
        self.button_start.clicked.connect(self.get_weather)
    ...
    def get_weather(self):
        if self.request.text():
            try:
                ...
                observation = manager.weather_at_place(self.request.text())
                weather = observation.weather
                temp = weather.temperature('celsius')
                self.result = f"""Погода: {weather.detailed_status} 
Температура: {temp['temp']}°С
Ощущается: {temp['feels_like']}°С
