...
class Wiki(QWidget):
    ...
    def initUI(self):
        ...
        self.request.setPlaceholderText("Текст запроса")
        self.info = QTextEdit()
        self.info.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
        self.info.setReadOnly(True)
        self.button_start = QPushButton("Искать")
    ...
    def connects(self):
        ...
        self.button_start.clicked.connect(self.get_wiki)
    ...
    def get_wiki(self):
        if self.request.text():
            try:
                ...
                self.result = wikipedia.page(self.request.text())
                self.info.setText(self.result.summary)
            except:
